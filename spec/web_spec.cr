...
private def get(path, params = nil, headers = nil)
  resource = "#{path}?#{params.try(&.map { |k, v| "#{URI.encode_www_form(k)}=#{URI.encode_www_form(v)}" }.join("&"))}"
...
private def post(path, params = nil, headers = nil)
  resource = path
  body = params.try(&.map { |k, v| "#{URI.encode_www_form(k)}=#{URI.encode_www_form(v)}" }.join("&"))
  hdrs = HTTP::Headers.new
  headers.each do |k, v|
    hdrs[k] = v
  end if headers
